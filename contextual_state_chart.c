...
int StateGetInt(ContextualStateChart* contextualStateChart, Vector* parent_name, Vector* variable_name)
{
	...
	// my_state->keys
	// partial match and make sure the ascii extra symbols are not aphabetical
	// if we use a unique var name within our parent state then the rest of the trie nodes the user didn't type in(the autogenerated ones)
	// will have only 1 edge
